...
All the project work, codes and graphics are available under [Project 3](https://github.com/greeneyefirefly/MSDS-DATA-607-Project-3) Github.
...
We required any reliable sources that lists renowned data scientists and analytical thinkers of our time. This lead to the following inclusion criteria which all possible websites sources must fulfill before we commenced data extraction:  

* has a professional profile (or link to an active profile) for the leader.
* has Identified the leader's geographic location.
* has description of their area of expertise as it related to data science.

Based on this task, we identified the following steps, and mapped our procedures in order to successfully accomplish our aim for this project.
...
We ran into a few concerns during this search, primarily in regards to that there was no definite source which could supply all the needed information. Therefore, we decided to pull necessary information from various additional sources such as Twitter, LinkedIn, Blogs and other online articles. 

Twitter and LinkedIn proved to be out best source for additional information before of their features.

***
<center> *Twitter* </center>   
Twitter is social networking site with more than 560 million active users. On average, 5700 tweets are posted every second.  Due to its in real time interaction, Twitter allows users to report on what is going on during an event can happen as the incident unfolds.

<center> *LinkedIn* </center>   
LinkedIn is a more business oriented social networking site. There are more that 240 million active users, most of whom are of in the workforce. This platform a source of information that pertains to professional fields and insights. Here, too, there are various components that are of interest which can be extracted.  

***
We agreed these will provide the information we needed to conduct our tasked analysis, in addition to containing other variables we thought would be interesting to investigate.
...
***
##### Data Collection
Retrieving correct information from the source website was a important task. HTML was extracted from Chrome using webscrapper and then data was extracted using R. Once the profile links were obtain, extraction from Twitter and LinkedId could commence.
...
* Area of Expertise: topics in which the user have work experience.
* Industry of Work: the general category under which an occupation falls that the user have experiences working in.

The following is a sample of how the data was read from the respective sources. The full code can be found on the [Project 3](https://github.com/greeneyefirefly/MSDS-DATA-607-Project-3) Github. 

<details>
  <summary> *Sample Code on Reading LinkedIn Data* </summary>
...
We decided to store the extracted data sets onto MongoDB Atlas (Cloud). We noticed that MongoDB provides a package for R, `mongolite`, which provides APIs required to connect to cloud and perform data operations.

<details>
  <summary> *Sample Code on Retrieving Data* </summary>
```{r eval=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
tweet <- read_csv("tweet.csv")
dataurlA <- read.csv("C:/Users/Deokinanan/Desktop/2019 -2021 CUNY SPS/SPRING 2019/DATA 607/Week 8/dataurlA.csv", header=TRUE, stringsAsFactors = FALSE)
linkedin <- read.csv("C:/Users/Deokinanan/Desktop/2019 -2021 CUNY SPS/SPRING 2019/DATA 607/Week 8/linkedin.csv", header=TRUE, stringsAsFactors = FALSE)
```
...
Our data collection enabled us to built the follow table of more than 50 of today's data science influencers from around the world. These stars of the business are not just a source of inspiration to data science professionals and aspirants alike but also ensure that you keep abreast of all new developments. Their content is valuable, informative, engaging, and, more importantly, relevant to today's data and technology discussions.

This table provides the links to their Twitter account so you can follow and keep up with the current trends and opportunities in data science and predictive analytics.
...
***  
The following map of the world locates our thought leaders, and highlight the magnitude of their popularity (# of followers, indicated by the size of the circle) on Twitter. You will notice is that most of the thought leaders are located in the Midwest-Northeast of the United States of America, but in the United States overall. This may be a result of search bias since as only English websites are scrapped, and Twitter is mostly popular in the United States than any other country in the world. 

However, we were still able to find a few from other countries, this shows that there are definitely more thought leaders in data science from all around the world even if they are not shown here. Lastly, it is also good to note that the number of followers is one number you should not correlate with the impact of influence these experts possess because some of least followed leaders have also made great contributions in the fields of data science.
...
Therefore, content related to these tags were frequently being discussed on that day, and are some of the things data scientists are currently talking about. It was determined from the tweets that the *Top 20 Hashtags* mentioned are highlighted below. 
...
hashtags <- tolower(gsub("[#]","", tweetRTremoved$hash))
...
ggplot(hashtags, aes(x = hashtags, y = n)) +
   ...
   labs(y = "Frequency",
        x = "Hashtags",
        title = "Top 20 Hashtags Found in Tweets")
```

***

#### Prerequisite Skills
##### Can I become a "thought leader" in Data Science?

We now know what are the hot topics being discussed, however, Team SPARC got more curious. What can we determined from our extracted data about the skill sets and knowledge these data scientists possess which made them successful today? 

This is where their LinkedIn profile aided us to answer this question. These influencers in the field have the ability to not only manipulate vast amounts of data with advanced statistical and visualization techniques, but have an in-demand acumen from which they can derive excellent insights. By analyzing their listed skills on their profile, the *Top 5 Skills Data Scientists Possess* are shown below.
...
ggplot(skills, aes(x = skills, y = (100*(n/sum(n))))) +
   ...
   labs(y = "Percentage",
        x = "Skills",
        title = "Top 5 Data Science Skills to Possess")
```

As a result, *strategy* is the most referenced ability they are skilled in. Followed by knowledge of *enterprise software*, *digital transformation*, *leaderships* and *business intelligence*. It is important to note that these are technical and non-technical skills, both of which are important and compliment each other to perform in an effective manner. Moreover, the graph below further highlights and narrows these skills into the *Top 10 Areas of Expertise* that are directly linked to the discipline of industrial design.
...
ggplot(Industry, aes(x = Industry, y = (100*(n/sum(n))))) +
   ...
   labs(y = "Percentage",
        ...
        title = "Top 10 Industry of Experience")
```

As a result, many have professional experiences working in *financial services and banking, professional services, technology, telecommunications, healthcare*, etc.

This shows that there are many companies who are trying to make sense of vast amount of data available. The pioneers have been around for a while but the most of the scientists today have been on the job for only a few years. It's a good time to enter the field for those who want also successfully contribute in this fresh and exciting field. As stated by the United States of Labor Statistics, nearly 80% of data scientists report a shortage in their field and the projected growth over the next decade is at 11%, which is higher than the 7% estimated growth for all occupations.

***
### Conclusion

In conclusion, we were able to build a table of more than 50 of today's data science influencers from around the world. These experts are a source of inspiration and actively keep up with all new developments in artificial intelligence, digital transformation, big data, analytical software, machine learning and many more. Their hard and soft skills are what is currently in-demand in the workforce. Excellent strategy and leaderships skills are the greatly needed in terms on non-technical skills, where as knowledge of enterprise software, digital transformation and business intelligence are the important technical skills. 

With a promising outlook for occupations in data science in the coming years, one can advance their career by seeking opportunities by in financial services and banking, professional services, technology, telecommunications, healthcare, etc and begin gaining valuable professional experiences. Maybe one day if we execute this project again, we may find your name here amongst the list of the thought leaders in data science of the 21st century.
...
* How can just one source be trusted?
	+ Identified the most commonly mentioned profiles amongst all the sources and discussed which one to select as trusted source. * What if we are given conflicting information from each selected site?
	+ Decided to exclude outlier information which was not proposed by majority sites.

***
#### Data Collection
* Some of the sites were restricted for data access unless authenticated and some API's were restricted by limited number of API calls. 
...
***
#### Storage & Retreive
* Only one of our team member (Rajwant) was aware of MongoDB Atlas feature, and rest weren't unaware.
	+ Rajwant took a KT session to team to explain the process of connecting to cloud and performing operations.
	+ Credentials were shared with team members with limited time access.  
	
***
#### Data Analysis
* Some strings were encrypted in unicode and were not accounted for when conducting analysis.
	+ Further reading was done how to effectively remove unicode using `stringr`.
